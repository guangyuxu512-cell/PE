  ...
  ipcMain.handle('ai-generate-title', async (_, { cfg, title, category, props }) => {
    ...
    return chatJson(cfg, [
      ...
      },
      {
        role: 'user',
        content:
          '请根据以下商品信息，生成 5 个适合电商平台的商品标题。\n'
          + '严格要求：\n'
          + '1. 每个标题必须在 55-60 个中文字符之间，尽可能接近 60 字，不能少于 55 字\n'
          + '2. 不能删除原标题中的任何关键词，只能重新排列组合\n'
          + '3. 如果标题不足 60 字，必须补充相关搜索热词，如使用场景、材质特点、适用人群等，尽量凑满 60 字\n'
          + '4. 不要包含价格、数量、库存、元起等信息\n'
          + '5. 不要使用标点符号，纯关键词堆叠\n'
          + `原标题：${title || '无'}\n`
          + `商品分类：${category || '无'}\n`
          + `核心属性关键词：${keywords}\n`
  ...
  ipcMain.handle('ai-optimize-title', async (_, { cfg, title }) => {
    return chatJson(cfg, [
      ...
      },
      {
        role: 'user',
        content:
          '你是电商标题优化专家。请根据以下原始标题，生成 5 个优化版本。\n'
          + '严格要求：\n'
          + '1. 每个标题必须在 55-60 个中文字符之间，尽可能接近 60 字，不能少于 55 字\n'
          + '2. 不能删除原标题中的任何关键词，只能重新排列组合\n'
          + '3. 如果原标题不足 60 字，必须补充相关搜索热词，如使用场景、材质特点、适用人群等，尽量凑满 60 字\n'
          + '4. 不要包含价格、数量、元起等信息\n'
          + '5. 不要使用标点符号，纯关键词堆叠\n'
          + `原始标题：${title || ''}\n`
          + '只返回 JSON 数组，包含 5 个字符串。',
      },
    ], 'array')
  })
